...
var betterWhitelist = {
    ...
    author: '9Yan',
    description:
        '更完善的白名单管理方案，基于Serein成员管理，需禁用白名单相关正则',
};
serein.registerPlugin(
    betterWhitelist.name,
    betterWhitelist.version,
    betterWhitelist.author,
    betterWhitelist.description
);
...
function hasPermission(userID) {
    return Boolean(
        serein.getSettingsObject().bot.permissionList.indexOf(userID) + 1
    );
}
...
function isGroup(groupID) {
    return Boolean(
        serein.getSettingsObject().bot.groupList.indexOf(groupID) + 1
    );
}
...
function syncWhitelist(groupID) {
    ...
    whitelistPath = serein
        .getSettingsObject()
        .server.path.replace(IS_PATH, 'allowlist.json');
    ...
    setTimeout(() => {
        ...
        var newIds = members.data.filter(
            (item) => !oldIds.includes(item.gameID)
        );
        newIds.forEach((item) => {
    ...
    (groupID, userID, msg, shownName) => {
        ...
        if (config.command.whitelist.keywords.includes(keyword)) {
            ...
            whitelistPath = serein
                ...
                .server.path.replace(IS_PATH, 'allowlist.json');
            if (!FILE.Exists(whitelistPath)) {
                whitelistPath = whitelistPath.replace(
                    IS_PATH,
                    'whitelist.json'
                );
            }
            ...
            for (let i = 0; i < members.data.length; i++) {
                ...
                let isName = members.data[i].card
                    ? members.data[i].card
                    : members.data[i].nickname
                    ? members.data[i].nickname
                    : members.data[i].ID;
                Array.push({
                    type: 'node',
            ...
            if (config.sendGroup) {
                while (Array.length > 90) {
                    serein.sendPacket(
                        '{"action": "send_group_forward_msg","params": {"group_id": "' +
                            groupID +
                            '","messages": ' +
                            JSON.stringify(Array.splice(0, 90)) +
                            '}}'
                    );
                }
                serein.sendPacket(
...
serein.setListener('onGroupDecrease', (groupID, userID) => {
    if (!config.exitGroup || !isGroup(groupID) || isIgnoreGroup(groupID))
        return;

    if (config.sendGroup) {
        serein.sendGroup(
            groupID,
            '群成员 ' + userID + ' 退群了，尝试删除白名单'
        );
    }
...
serein.setListener('onServerOutput', (msg) => {
    ...
    if (notGameID.indexOf(gameID) == -1) {
        ...
        for (
            let i = 0;
            i < serein.getSettingsObject().bot.groupList.length;
            i++
        ) {
            if (!isIgnoreGroup(serein.getSettingsObject().bot.groupList[i])) {
                var groupID = serein.getSettingsObject().bot.groupList[i];
...
setInterval(() => {
    for (let i = 0; i < notGameID.length; i++) {
        serein.sendCmd(
            'kick "' + notGameID[i] + '" You do not have a whitelist!'
        );
    }
}, 1000);
